...
const webhookHandlerFilePath = path.join(
  process.cwd(),
  "server",
  "webhooks",
  "_index.js"
...
async function writeToApi() {
  try {
    ...
    const webhookImports = shopifyFileContent.match(
      /import .* from "\.\.\/server\/webhooks\/.*";/g
    );

    let webhookHandlerFileContent = fs.readFileSync(
      webhookHandlerFilePath,
      "utf8"
    );
 ...
 */\n\n`;
    ...
    // Start with a fresh file containing shopify import first
    let newFileContent =
      topComment + 'import shopify from "../../utils/shopify.js";\n';

    // Add other imports from webhook handlers if they exist
    if (webhookImports) {
      const uniqueImports = [...new Set(webhookImports)];
    ...
    // Get the rest of the file content after the imports
    const mainContent = webhookHandlerFileContent.replace(
      /^([\s\S]*?^import[^\n]*\n)+/m,
      ""
    );

    // Combine everything
    webhookHandlerFileContent = newFileContent + mainContent;
    ...
    switchCaseStatement += `    throw new Error(\`Can't find a handler for \${${
      hasDuplicateTopics ? "req.path" : "validateWebhook.topic"
    }}\`);\n`;
    switchCaseStatement += "}\n";

    // Replace the existing switch/case statement
    const switchCaseRegex = /\/\/SWITCHCASE\n[\s\S]*?\/\/SWITCHCASE END/;
    webhookHandlerFileContent = webhookHandlerFileContent.replace(
      switchCaseRegex,
      `//SWITCHCASE\n${switchCaseStatement}//SWITCHCASE END`
    );

    fs.writeFileSync(webhookHandlerFilePath, webhookHandlerFileContent, "utf8");
  } catch (error) {
    console.error("Error writing to webhookHandler file:", error);
  }
}
